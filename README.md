...
> 交付日期：2026-09-14　|　实例：AutoDL west-B 区　|　状态：**验收通过（10/11 项能力可用）**
> 本文档面向**复现者与接手人**。原部署过程另有两份内部文档（`交付报告.md` 正式报告、
> `部署进度与交接.md` 全过程记录与踩坑），因含实例信息未纳入本仓库。
...
```
Virtual-Human/                 # ← 本仓库（不含权重，约 350 KB）
├── README.md                  #   本文件
...
```
        ...
        │  · GPT-SoVITS（声音克隆）              │   │  · MuseTalk（⚠️ 无口型动作）      │
        └────────────────────────────────────────┘   └───────────────────────────────────┘
                                     │                                  │
                                     └──────────────┬───────────────────┘
                                                    ▼
...
| MuseTalk UNet | `Musetalk/models/musetalk/pytorch_model.bin` | 3.2 GB | ⚠️ **无口型动作** |
...
### 9.2 功能大盘点（`deploy/audit_capabilities.py`，10/11）
...
- 运动阈值 **0.5** —— 把产物首帧用同样编码参数做成静帧视频测基线得 **0.01**，
  真实产出 **1.31 ~ 2.93**。取 0.5 高于静帧 50 倍、低于最低真视频 2.6 倍。
...
### 10.5 MuseTalk：一个未被解决的失败

MuseTalk 是「实时对话」路径的引擎。现象：**任务成功、mp4 合法，但嘴完全不动。**

排查过程（`deploy/diag_musetalk*.py`）：

| 环节 | 观测 | 判断 |
| --- | --- | --- |
| 准备阶段 | 100 帧、人脸框合理、mask 非零 33%、latents 正常 | ✅ |
| Whisper 音频特征 | 无 nan/inf，相邻 chunk 逐元素平均差 0.237 | ✅ 在变 |
| UNet 输入条件 | 逐批 `std` 极差 **1.53**（变化很大） | ✅ 进了模型 |
| **UNet 输出** | 逐批 `std` 极差 **0.008**（约 1%） | ❌ **对条件几乎无响应** |
...
> ⚠️ **一个自我纠错**：第一次判定时用的音频正好是**静音的**（问题 2 尚未修复），
> 那个证据是污染的。用真实音频重测后才确认——**仍然不动**，但结论依据换成了干净的。

**状态：根因未定位，未修复。** 服务默认走 SadTalker，`mode=auto` 也解析为 SadTalker，
不影响六条验收标准。另发现一个真 bug 但不在我们路径上：本分支的 `get_landmark_and_bbox`
返回 **3** 个值，`TFG/MuseTalk.py:281` 只接 2 个（那个函数是坏的，我们走的是另一个）。
...
| # | 限制 | 影响 | 建议 |
| --- | --- | --- | --- |
| 1 | **MuseTalk 无口型动作** | 实时对话标签页不可用 | 见 11.5；SadTalker / Wav2Lip 可完全替代 |
...
### 11.2 继续排查 MuseTalk 的起点

1. 对照上游 MuseTalk 官方仓库，确认 `musetalk/models/musetalk/pytorch_model.bin`
   与**本分支代码**是否配套（本分支的 `preprocessing.py` 已被 Linly-Talker 改过）；
2. 用上游原始 `musetalk` 包 + 官方 `scripts/inference.py` 跑一次，隔离「模型问题」与「封装问题」；
3. 重点看 `TFG/MuseTalk.py` 的 `prepare_material` 里 latent 的构造，
   与 UNet 期望的 8 通道输入是否语义一致。
